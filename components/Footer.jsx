...
const Footer = ({}) => {
  ...
  const [isSubscribeText, setSubscribeText] = useState('Subscribe')
  ...
  const adduser = (e) => {
    ...
    base("newsletter").create(
        ...
        function(err) {
            if (err) {
                setLoading(false)
                console.log('This is airtable record error', err)
                return;
            }
  ...
  <div className="max-w-md mx-auto">
      ...
      {isLoading ? 
       (<button  type="button" className="absolute top-1/2 right-1.5 -translate-y-1/2 rounded-full bg-blue-600 p-3 text-xs font-bold uppercase tracking-wide text-white transition hover:bg-blue-700">
    ...
    {/* <nav className="mt-12" aria-labelledby="footer-navigation">
      ...
      <ul className="flex flex-wrap justify-center gap-6 md:gap-8 lg:gap-12">
        <li>
          <p className="text-gray-700 transition hover:text-gray-700/75" href="/">
           List
          </p>
        </li>
...
export async function getServerSideProps() {
    ...
    return {
       props: ({ base})  // will be passed to the page component as props
    }
  }